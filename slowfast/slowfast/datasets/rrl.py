...
class Rrl(torch.utils.data.Dataset): 
    """
    RRL video loader. Construct the rrl video loader, then sample
    ...
    def __init__(self, cfg, mode, transform=None):
        """
        Construct the RRL video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        """
        # Only support train, eval, val, and test mode.
        assert mode in [
            "train",
            "eval",
            "val",
            "test",
        ], "Split '{}' not supported for RRL".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        self.use_chunk_loading = (
            True
            if self.mode in ["train", "eval"] and self.cfg.DATA.LOADER_CHUNK_SIZE > 0
            else False
        )
        self.dummy_output = None
        self.transform = transform
        
        # For training, evaluation or validation mode, one single clip is sampled from every
        # video. For testing, NUM_ENSEMBLE_VIEWS clips are sampled from every
        # video. For every clip, NUM_SPATIAL_CROPS is cropped spatially from
        # the frames.
        if self.mode in ["train", "eval", "val"]:
            self._num_clips = 1
        ...
        logger.info("Constructing RRL {}...".format(mode))
        ...
        self.cur_epoch = 0
        self.soft_labels = torch.zeros(self.num_videos, cfg.MODEL.NUM_CLASSES)
        self.v_centers = torch.zeros(cfg.MODEL.NUM_CLASSES, 48)

        if self.mode in ["train", "eval"] and self.cfg.AUG.ENABLE:
            self.aug = True
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load RRL split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing rrl dataloader (size: {} skip_rows {}) from {} ".format(
        ...
        for i_try in range(self._num_retries):
            ...
            for i in range(num_decode):
                for _ in range(num_aug):
                    ...
                    # Perform color jitter, random grayscale and gaussian blur
                    if (
                        self.mode in ["train", "eval"]
                    ...
                    relative_scales = (
                        None
                        if (self.mode not in ["train", "eval"] or len(scl) == 0)
                        else scl
                    )
                    relative_aspect = (
                        None
                        if (self.mode not in ["train", "eval"] or len(asp) == 0)
                        else asp
                    )
