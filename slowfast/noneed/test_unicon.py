...
@torch.no_grad()
def perform_test(test_loader, model1, model2, test_meter, cfg, writer=None):
    """
    For classification:
    ...
    for num_view in cfg.TEST.NUM_TEMPORAL_CLIPS:
        ...
        if du.is_master_proc() and cfg.LOG_MODEL_INFO:
            ...
            flops, params = misc.log_model_info(
                model1, cfg, use_train_input=False
            )

        if du.is_master_proc() and cfg.LOG_MODEL_INFO:
            misc.log_model_info(model1, cfg, use_train_input=False)
            misc.log_model_info(model2, cfg, use_train_input=False)
        # if (
        #     cfg.TASK == "ssl"
        ...
        cu.load_test_checkpoint_unicon(cfg, model1, model2)
        ...
        test_meter = perform_test(test_loader, model1, model2, test_meter, cfg, writer)
