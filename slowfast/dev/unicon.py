...
class Unicon(torch.utils.data.Dataset):
    """
    Kinetics video loader with Unicon. Construct the Kinetics video loader, then sample
    ...
    def __init__(self, cfg, mode, transform=None, sample_ratio=0.0, prob=None, noise=None):
        """
        Construct the Unicon-Kinetics video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        Args:
            cfg (CfgNode): configs.
            mode (string): Options includes `train`, 'warmup', `val`, or `test` mode.
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Unicon-Kinetics".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        self.use_chunk_loading = (
            True
            if self.mode in ["train", "warmup"] and self.cfg.DATA.LOADER_CHUNK_SIZE > 0
            else False
        )
        ...
        # the frames.
        if self.mode in ["train", "warmup", "val", "eval_train"]:
            self._num_clips = 1
        ...
        logger.info("Constructing Unicon-Kinetics {}...".format(mode))
        ...
        if self.mode in ["train", "warmup"] and self.cfg.AUG.ENABLE:
            self.aug = True
            if self.cfg.AUG.RE_PROB > 0:
                self.rand_erase = True
        
        if self.mode == "train":
            self._uniform_selection(prob, noise)
    ...
    def _construct_loader(self):
        ...
        """
        if self.mode not in ['warmup', 'eval_train']:
            path_to_file = os.path.join(
                self.cfg.DATA.PATH_TO_DATA_DIR, "{}.csv".format(self.mode)
            )
        else:
            path_to_file = os.path.join(self.cfg.DATA.PATH_TO_DATA_DIR, "train.csv")          
        ...
        with pathmgr.open(path_to_file, "r") as f:
            ...
            for clip_idx, path_label in enumerate(rows):
                ...
                for idx in range(self._num_clips):
                    ...
                    self.class_ind[int(label)].append(clip_idx)
                    self._spatial_temporal_idx.append(idx)
                    self._video_meta[clip_idx * self._num_clips + idx] = {}
        self.num_videos_all = len(self._path_to_videos)
        assert (
    ...
    def __getitem__(self, index):
        ...
        # num_decode = 1 for all the modes in unicon dataset
        num_decode = (
        ...
        if len(min_scale) < num_decode:
            ...
            assert self.mode in ["train", "warmup", "val", "eval_train"]
        ...
        for i_try in range(self._num_retries):
            ...
            if self.cfg.DATA.TRAIN_JITTER_FPS > 0.0 and self.mode in ["train", "warmup"]:
            ...
            # frames_decoded_s1 = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded)
            # frames_decoded_s2 = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded)
            
            # frames_decoded_aug = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded) 
            ...
            #             tw1, tw2, ts1, ts2 = self.transform
            #             frames_decoded_w1[i], frames_decoded_w2[i], frames_decoded_s1[i], frames_decoded_s2[i] = tw1(frames_decoded[i]), tw2(frames_decoded[i]), ts1(frames_decoded[i]), ts2(frames_decoded[i])
            #             frames_decoded[i] = frames_decoded[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            #             frames_decoded_w1[i] = torch.stack(frames_decoded_w1[i], dim=0).permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            #             frames_decoded_w2[i] = torch.stack(frames_decoded_w2[i], dim=0).permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            #             frames_decoded_s1[i] = torch.stack(frames_decoded_s1[i], dim=0).permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            #             frames_decoded_s2[i] = torch.stack(frames_decoded_s2[i], dim=0).permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)

            # elif self.mode in ['warmup', 'eval_train', 'test', 'val']:
            #     for i in range(len(frames_decoded)):
            #         # Apply transformations
            #         if self.transform is not None:
            #             # print("The shape of frames_decoded[i] before transform is ", frames_decoded[i].shape)
            #             frames_decoded[i] = frames_decoded[i].permute(0, 3, 1, 2) # Convert (T, H, W, C) to (T, C, H, W)
            #             frames_decoded_aug[i] = self.transform(frames_decoded[i])
            #             frames_decoded_aug[i] = torch.stack(frames_decoded_aug[i], dim=0).permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            #             frames_decoded[i] = frames_decoded_aug[i]
            ...
            # num_aug == 2 if mode in [train, warmup]
            num_aug = (
                self.cfg.DATA.TRAIN_CROP_NUM_SPATIAL * self.cfg.AUG.NUM_SAMPLE
                if self.mode in ["train", "warmup"]
                else 1
            )
            num_out = num_aug * num_decode  # num_out = 2 if mode = train or warmup else 1
            f_out, time_idx_out = [None] * num_out, [None] * num_out
            idx = -1
            label = self._labels[index]

            # if self.mode == 'train':
            #     frames_decoded_train = [frames_decoded_w1, frames_decoded_w2, frames_decoded_s1, frames_decoded_s2]
                
            #     f_out_train = [[None] * num_out, [None] * num_out, [None] * num_out, [None] * num_out]
                
            #     for i, frames_decoded_t in enumerate(frames_decoded_train):
            #         for j in range(num_aug):
            #             f_out_train[i][j] = frames_decoded_t[0].clone()
            ...
            #             ):
            #                 f_out_train[i][j] = transform.color_jitter_video_ssl(
            #                     f_out_train[i][j],
            ...
            #                     input_size=(f_out_train[i][j].size(1), f_out_train[i][j].size(2)),
            #                     auto_augment=self.cfg.AUG.AA_TYPE,
            #                     interpolation=self.cfg.AUG.INTERPOLATION,
            #                 )
            #                 # T H W C -> T C H W.
            #                 f_out_train[i][j] = f_out_train[i][j].permute(0, 3, 1, 2)
            #                 list_img = self._frame_to_list_img(f_out_train[i][j])
            #                 list_img = aug_transform(list_img)
            #                 f_out_train[i][j] = self._list_img_to_frames(list_img)
            #                 f_out_train[i][j] = f_out_train[i][j].permute(0, 2, 3, 1)

            #             # Perform color normalization.
            #             f_out_train[i][j] = utils.tensor_normalize(
            #                 f_out_train[i][j], self.cfg.DATA.MEAN, self.cfg.DATA.STD
            #             )

            #             # T H W C -> C T H W.
            #             f_out_train[i][j] = f_out_train[i][j].permute(3, 0, 1, 2)
            ...
            #                 else asp
            #             )
            #             f_out_train[i][j] = utils.spatial_sampling(
            #                 f_out_train[i][j],
            ...
            #                     device="cpu",
            #                 )
            #                 f_out_train[i][j] = erase_transform(
            #                     f_out_train[i][j].permute(1, 0, 2, 3)
            #                 ).permute(1, 0, 2, 3)
                        
            #             # f_out[j] = torch.cat((f_out[j], f_out_train[i][j]), dim=0)
            #             time_idx_out[j] = time_idx_decoded[0, :]
                        
            #     for n in range(num_aug):
            #         f_out[n] = torch.cat((f_out_train[0][n], f_out_train[1][n], f_out_train[2][n], f_out_train[3][n]), dim=0)
            #         f_out[n] = utils.pack_pathway_output(self.cfg, f_out[n])
    ...
    def _uniform_selection(self, probability, noise):
        pred_idx  = np.zeros(int(self.sample_ratio * self.num_videos_all))
        # class_len = int(self.sample_ratio * self.num_videos / cfg.MODEL.NUM_CLASSES)
        size_pred = 0
        ...
        for i in range(self.cfg.MODEL.NUM_CLASSES):            
            ...
            prob_idx  = np.argsort(probability[class_indices].cpu().numpy())
            class_size = int(len(class_indices) * self.sample_ratio)

            # try:
            pred_idx[size_pred:size_pred+class_size] = np.array(class_indices)[prob_idx[0:class_size].astype(int)].squeeze()
            size_pred += class_size
            # except:                            
        ...
        idx = list(range(self.num_videos_all))
        pred_idx_noisy = [x for x in idx if x not in pred_idx]        
        
        if noise == 'labeled':
            # self._path_to_videos = self._path_to_videos[pred_idx]
            self._path_to_videos = [self._path_to_videos[i] for i in pred_idx]
            # self._labels = self._labels[pred_idx]    
            self._labels = [self._labels[i] for i in pred_idx]  
        else:
            self._path_to_videos = [self._path_to_videos[i] for i in pred_idx_noisy]
            self._labels = [self._labels[i] for i in pred_idx_noisy]  
